...
The snippets are prohibitively long to directly include in this markdown file, but they are in this same repo. Peep the files `tex.snippets` and `greek_tex.snippets`. The former goes in `~/.vim/UltiSnips` directory, and the latter goes in `~/.vim/UltiSnips/greek/UltiSnips`; you'll have to `mkdir greek/UltiSnips` (which makes both, nested) from inside `~/.vim/UltiSnips` first.
